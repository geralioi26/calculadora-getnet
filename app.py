...
def guardar_en_google(cliente, vehiculo, detalle, p_venta, p_compra, proveedor, codigo):
    ...
    try:
        ...
        df_existente = pd.DataFrame(columns=["Fecha", "Cliente", "Vehículo", "Detalle", "Venta $", "Compra $", "Proveedor", "Código"])
    
    # Nuevo registro
    nuevo_reg = pd.DataFrame([[fecha_hoy, cliente, vehiculo, detalle, p_venta, p_compra, proveedor, codigo]], 
...
# Lógica dinámica de textos (balanceado y sin paréntesis)
if tipo_kit == "Nuevo":
    ...
    m_neg = [f"*{m}*" for m in marcas_elegidas]
...
if foto is not None:
    try:
        # Conversión para evitar el ValueError rosa
...
if st.sidebar.button("💾 GUARDAR PARA SIEMPRE"):
    ...
    st.sidebar.success(f"¡Venta de {vehiculo} guardada en la nube!")
...
# --- 📜 HISTORIAL PERMANENTE (Desde Google Sheets) ---
st.divider()
st.subheader("📋 Historial en la Nube (No se borra)")
try:
    df = conn.read(worksheet="Ventas")
    if not df.empty:
        st.dataframe(df[::-1], use_container_width=True)
        ganancia = df["Venta $"].sum() - df["Compra $"].sum()
        st.info(f"💰 **Ganancia Total Acumulada: $ {ganancia:,.2f}**")
except Exception:
    st.info("Conectá tu Google Sheet en 'Secrets' para ver el historial.")
